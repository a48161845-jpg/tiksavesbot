...
async def stars_set_cmd(message: Message):
    ...
    who_label = await resolve_user_label(message.bot, uid)
    store.set_user_label(uid, who_label)
    new_value = store.set_user_stars(uid, value)
    ...
    log_admin(admin_id, "stars_set", f"target={uid} value={value}")
...
async def money_set_cmd(message: Message):
    ...
    who_label = await resolve_user_label(message.bot, uid)
    store.set_user_label(uid, who_label)
    new_value = store.set_user_money(uid, value)
    ...
    log_admin(admin_id, "money_set", f"target={uid} value={value}")
